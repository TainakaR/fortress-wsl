...
ご指摘ありがとうございます。まさにその通りですね。指導書（マニュアル）において「実行した後に何と聞かれ、どう答えるべきか」が書かれていないと、初学者はそこで手が止まって不安になってしまいます。

実行時のプロンプト（質問）への具体的な対処方法を追記し、誰もが迷わず進められるよう「3.4」の項目をブラッシュアップしました。
...
この作業は、Ubuntuの中ではなく、外側にあるWindowsのPowerShell**で行います。*(※すでに鍵を作成済みの場合はこの手順をスキップしてください)*
